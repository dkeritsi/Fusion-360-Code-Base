...
class Grid:
    ...
    def peano_curve(self):
        cell_instance_i = self.matrix_cell_instances[0,0]
        for x in range(0, x_row_grid_length):
            for y in range (0, y_column_grid_length):
               if(x!= 0 and y != 0):
                current_cell_instance_ii = self.matrix_cell_instances[x,y]
                link_cells(current_cell_instance_ii, cell_instance_i, self.sketchLines)
                cell_instance_i = current_cell_instance_ii

    def peano_curve_test(self, sketchLines):
        for x in range(0, self.x_row_grid_length):
            for y in range (0, self.y_column_grid_length):
                #self.link_cells(self.matrix_cell_centers[x][y+1], self.matrix_cell_centers[x][y], sketchLines)
...
def run(context):
    ...
    try:
        ...
        grid_instance.peano_curve_test(sketchLines)
