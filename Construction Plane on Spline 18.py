...
def create_body(radius, spline, construction_plane_for_face):
        ui  = adsk.core.Application.get().userInterface
        rootComp = adsk.core.Application.get().activeProduct.rootComponent
        sketches = rootComp.sketches

        #Sketch for Outter 
        # (1a) Create a Circle Profile at 0,0,0 of a Construction Plane
        circle_prof_outter = create_circle_prof(construction_plane_for_face, radius)

        # (1b) Create a sweep input that comprises a profile and a spline
        sweeps_outter = rootComp.features.sweepFeatures
        sweepInput_outter = create_sweep_input_for_new_body(spline, circle_prof_outter) 
        
        # (1c) Create the sweep.
        sweeps_outter.add(sweepInput_outter)
        ui.messageBox("Outter Sweeping Complete based on Circle and Spline Path.")

        #Sketch for Inner 
        # (1a) Create a Circle Profile at 0,0,0 of a Construction Plane
        #circle_prof_inner = create_circle_prof(construction_plane_for_face, radius)

        # (1b) Create a sweep input that comprises a profile and a spline
        #sweeps_inner = rootComp.features.sweepFeatures
        #sweepInput_inner = create_sweep_input_for_cut_body(spline, circle_prof_inner) 
        
        # (1c) Create the sweep.
        #sweeps_inner.add(sweepInput_inner)
        #ui.messageBox("Inner Sweeping Complete based on Circle and Spline Path.")
...
def run(context):
    ...
    try:
        ...
        planeInput, crvPath_spline, planes, sketches = create_body(radius, spline1, xyPlane)
        ...
        planeInput2, crvPath_spline2, planes2, sketches2 = create_body(radius, spline2, plane_2)
