...
class Grid:
    ...
    #matrix_cell_instances = null

    def __init__(self, x_row_grid_length_input, y_row_grid_length_input, sketchPoints_input, sketchLines_input, sketch_lines_draw_input:bool):
        ...
        #        self.matrix_cell_instances[x][y] = Cell(0, 0, 0, 0, self.sketchPoints, self.sketchLines)

        #self.matrix_cell_instances = [[0 for x in range(self.x_row_grid_length)] for y in range(self.y_column_grid_length)] 

    def populate_cells(self):

        #for x in range(0, self.x_row_grid_length):
        #    for y in range(0, self.y_column_grid_length):
        #        matrix_cell_instances[x][y] = Cell(0, 0, 0, 0, self.sketchPoints, self.sketchLines)
        row_matrix_cell_instances = [Cell(0,0,0,0, self.sketchPoints, self.sketchLines) for i in range(self.y_column_grid_length)]
        matrix_cell_instances = [list(row_matrix_cell_instances) for i in range (self.x_row_grid_length)]

        for x in range(0, self.x_row_grid_length):
            for y in range (0, self.y_column_grid_length):
                current_cell_instance = Cell(x, y, 0, 1, self.sketchPoints, self.sketchLines)
                #self.matrix_cell_instances[x,y] = current_cell_instance
                matrix_cell_instances = current_cell_instance
                if(self.sketch_lines_draw):
                    current_cell_instance.link_with_sketch(self.sketchLines) 
...
def run(context):
    ...
    try:
        ...
        grid_instance = Grid(x_row_grid_length, y_column_grid_length, sketchPoints, sketchLines, True)
