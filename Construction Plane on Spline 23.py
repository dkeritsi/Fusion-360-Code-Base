...
#Description- Piecewise Parametric Function Slicing
#Testing show editing
...
def create_pipe_body(inner_radius, outter_radius, spline, construction_plane_for_face):
        ...
        ui.messageBox("Pipe Body Completed.")

        return planeInput, crvPath_spline 

def construction_plane_from_three_points():
...
def run_along_spline_of_pipe(number_of_construction_planes, planeInput, crvPath_spline, outter_radius):
...
def points_of_spline(number_of_points):
    ...
    points = []
    spline = []
    return points_float_set, points, spline, number_of_points
...
def run(context):
    ...
    try:
        ...
        #First Path 
        points_float_set_1, points1, spline1, number_of_points_1 = points_of_spline(number_of_points = 4)
        ...
        #Second Path 
        points_float_set_2, points2, spline2, number_of_points_2 = points_of_spline(number_of_points = 4)
        ...
        #Third Path 
        points_float_set_3, points3, spline3, number_of_points_3 = points_of_spline(number_of_points = 4) 
        ...
        sketch_for_xy_plane_for_points1 = rootComp.sketches.add(rootComp.xYConstructionPlane)
        sketch_for_xy_plane_for_points2 = rootComp.sketches.add(rootComp.xYConstructionPlane)

        ####We will not use 3rd variable for now
        sketch_for_xy_plane_for_points3 = rootComp.sketches.add(rootComp.xYConstructionPlane)
        construction_of_splines(points1, points_float_set_1, spline1, number_of_points_1, sketch_for_xy_plane_for_points1)
        construction_of_splines(points2, points_float_set_2, spline2, number_of_points_2, sketch_for_xy_plane_for_points2) 
        construction_of_splines(points3, points_float_set_3, spline3, number_of_points_3, sketch_for_xy_plane_for_points2) 

        #Create Bodies 
        #Create First Body 
        xyPlane = rootComp.xYConstructionPlane
        planeInput, crvPath_spline = create_pipe_body(inner_radius, outter_radius, spline1, xyPlane)

        #Create Second Body
        plane_2 = construction_plane_from_three_points()
        planeInput2, crvPath_spline2 = create_pipe_body(inner_radius, outter_radius, spline2, plane_2)

        #Place Construction PLanes along Spline and Cut along First Body
        ui.messageBox("Start Slicing.")
        run_along_spline_of_pipe(number_of_construction_planes, planeInput, crvPath_spline, outter_radius)
        run_along_spline_of_pipe(number_of_construction_planes, planeInput2, crvPath_spline2, outter_radius)
