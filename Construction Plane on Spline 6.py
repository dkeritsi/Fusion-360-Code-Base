...
def run(context):
    ...
    try:
        ...
        #path = rootComp.features.createPath(spline[0])
        #sweeps = rootComp.features.sweepFeatures
        #sweepInput = sweeps.createInput(circle_prof, path, adsk.fusion.FeatureOperations.CutFeatureOperation)
        #sweepInput.guideRail = guide
        #sweepInput.profileScaling = adsk.fusion.SweepProfileScalingOptions.SweepProfileScaleOption
        
        # Create the sweep.
        #sweep = sweeps.add(sweepInput)
        normal_vectors = [] #We want an array of normal vectors
        ...
        for i in range (100):
            ...
            #extrudeInput = extrudes.createInput(sweep, adsk.fusion.FeatureOperations.CutFeatureOperation)
            #extrudeInput.setDistanceExtent(False, distance)
            #extrudes.add(extrudeInput)

            plane_i = planes.add(planeInput)
            #Add new Sketch
            rootComp.sketches.add(plane_i)


        ui.messageBox("Construction Planes running along Spline Complete.")

        #Perform Dot Product while Looping along the Layers
